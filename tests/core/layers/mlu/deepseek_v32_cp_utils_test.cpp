...
using v32_cp::all_gather_across_ranks;
using v32_cp::build_deepseek_v32_cp_context;
using v32_cp::build_local_prefill_attention_metadata;
using v32_cp::build_segment_ctx_lens_tensor;
using v32_cp::build_segment_length_matrix;
using v32_cp::DeepseekV32CPContext;
using v32_cp::finish_all_gather_across_ranks;
using v32_cp::launch_all_gather_across_ranks;
using v32_cp::pad_to_cp_rows;
using v32_cp::reorder_by_index;
using v32_cp::reorder_to_local_shard;
using v32_cp::restore_gathered_to_global_order;
using v32_cp::slice_local_packed;
namespace {
...
std::vector<int32_t> extract_segment_req_idx(
    const std::vector<DeepseekV32CPSegment>& segments) {
...
TEST(DeepseekV32CPUtilsTest,
     BuildZigzagSplitPlanMatchesSingleRequestWithoutPadding) {
  const auto all_segments = build_all_cp_segments(4, {16}, {16});
  const auto runtime_artifacts =
      build_cp_runtime_artifacts(0, 4, all_segments, /*total_tokens=*/16);
  ...
  const auto segments = build_local_cp_segments(0, all_segments);
...
TEST(DeepseekV32CPUtilsTest, BuildContextFailsFastForDecodeBatches) {
  ...
  for (const BatchForwardType batch_type :
    ...
    EXPECT_DEATH(build_deepseek_v32_cp_context(/*cp_size=*/2,
                                               attn_metadata,
                                               batch_type,
                                               tokens,
                                               &process_group,
...
TEST(DeepseekV32CPUtilsTest,
  ...
  const auto all_segments = build_all_cp_segments(4, {10}, {10});
  const auto runtime_artifacts =
      build_cp_runtime_artifacts(2, 4, all_segments, /*total_tokens=*/10);
  ...
  const auto segments = build_local_cp_segments(2, all_segments);
  EXPECT_EQ(extract_segment_req_idx(segments), (std::vector<int32_t>{0, 0}));
...
TEST(DeepseekV32CPUtilsTest, BuildZigzagSplitPlanTracksContextLens) {
  ...
  const auto all_segments = build_all_cp_segments(4, {4, 6}, {16, 22});
  const auto segments = build_local_cp_segments(1, all_segments);
...
TEST(DeepseekV32CPUtilsTest,
     BuildDeepseekV32CPContextBuildsMetadataForCurrentRank) {
  ...
  auto maybe_context =
      build_deepseek_v32_cp_context(/*cp_size=*/4,
                                    attn_metadata,
                                    BatchForwardType::PREFILL,
                                    tokens,
                                    reinterpret_cast<ProcessGroup*>(0x1),
...
TEST(DeepseekV32CPUtilsTest,
     BuildDeepseekV32CPContextFallsBackWhenSeqShorterThanWorldSize) {
...
TEST(DeepseekV32CPUtilsTest,
     BuildDeepseekV32CPContextAcceptsChunkedPrefillBatch) {
  ScopedFlagValue world_size_flag(DistributedConfig::get_instance().nnodes(),
                                  4);
...
TEST(DeepseekV32CPUtilsTest, BuildCPContextTracksSegmentRuntimeView) {
  ScopedFlagValue world_size_flag(DistributedConfig::get_instance().nnodes(),
                                  4);
...
TEST(DeepseekV32CPUtilsTest, BuildCPContextDerivesSegmentRowsForDcpMetadata) {
  AttentionMetadata attn_metadata =
...
TEST(DeepseekV32CPUtilsTest, BuildSegmentTensorCacheTracksPartialPrefixHit) {
  AttentionMetadata attn_metadata = make_prefill_metadata({4, 6}, {16, 22});
  const auto all_segments = build_all_cp_segments(4, {4, 6}, {16, 22});
  const auto local_segments = build_local_cp_segments(1, all_segments);
  ...
  EXPECT_TRUE(
      torch::equal(build_segment_length_matrix(local_segments,
                                               &DeepseekV32CPSegment::ctx_k_len,
  ...
  EXPECT_TRUE(torch::equal(
      build_segment_length_matrix(local_segments,
                                  &DeepseekV32CPSegment::suffix_k_len,
                                  torch::Device(torch::kCPU)),
      torch::tensor({{0, 2}, {0, 0}, {0, 2}, {0, 5}},
                    torch::TensorOptions().dtype(torch::kInt32))));
}

TEST(DeepseekV32CPUtilsTest, BuildCPContextKeepsExactBlockRollbackLens) {
  ScopedFlagValue world_size_flag(DistributedConfig::get_instance().nnodes(),
                                  4);
...
TEST(DeepseekV32CPUtilsTest, BuildCPContextTracksMixedHitMiss) {
  AttentionMetadata attn_metadata = make_prefill_metadata(
...
TEST(DeepseekV32CPUtilsTest, SliceLocalPackedUsesRankOffset) {
...
TEST(DeepseekV32CPUtilsTest,
     BuildDeepseekV32CPContextReturnsNulloptWhenDisabled) {
  ScopedFlagValue world_size_flag(DistributedConfig::get_instance().nnodes(),
                                  4);
  ...
  auto maybe_context =
      build_deepseek_v32_cp_context(/*cp_size=*/1,
                                    attn_metadata,
                                    BatchForwardType::PREFILL,
                                    tokens,
                                    reinterpret_cast<ProcessGroup*>(0x1),
...
TEST(DeepseekV32CPUtilsTest, ReorderByIndexSlicesFirstDimension) {
...
TEST(DeepseekV32CPUtilsTest, RestoreGatheredToGlobalOrderWithoutPadding) {
...
TEST(DeepseekV32CPUtilsTest, AllGatherAcrossRanksRestoresGlobalOrder) {
  ScopedFlagValue nnodes(DistributedConfig::get_instance().nnodes(), 4);
...
TEST(DeepseekV32CPUtilsTest, GatherSupportsUnevenThreeDimensionalInputs) {
  std::vector<torch::Tensor> scripted_outputs;
  scripted_outputs.reserve(4);
  for (int64_t world_rank = 0; world_rank < 4; ++world_rank) {
    scripted_outputs.push_back(
...
TEST(DeepseekV32CPUtilsTest, LaunchAndFinishGatherMatchBlockingGather) {
  std::vector<torch::Tensor> scripted_outputs;
  scripted_outputs.reserve(4);
  for (int64_t world_rank = 0; world_rank < 4; ++world_rank) {
    scripted_outputs.push_back(
...
TEST(DeepseekV32CPUtilsTest, PadToCPRowsExpandsLocalShardToPaddedRows) {
  ScopedFlagValue nnodes(DistributedConfig::get_instance().nnodes(), 4);
  ...
  torch::Tensor local_tensor = torch::tensor(
      {2.0f, 3.0f, 8.0f}, torch::TensorOptions().dtype(torch::kFloat32));
  torch::Tensor padded = pad_to_cp_rows(local_tensor, context);
...
TEST(DeepseekV32CPUtilsTest, CPContextBuildsGatheredSlotMapping) {
  ScopedFlagValue nnodes(DistributedConfig::get_instance().nnodes(), 4);
...
TEST(DeepseekV32CPUtilsTest,
  ...
  auto maybe_context =
      build_deepseek_v32_cp_context(/*cp_size=*/2,
                                    attn_metadata,
                                    BatchForwardType::PREFILL,
                                    tokens,
                                    reinterpret_cast<ProcessGroup*>(0x1),
...
TEST(DeepseekV32CPUtilsTest, AsyncGatherReturnsGatheredOrder) {
  ScopedFlagValue nnodes(DistributedConfig::get_instance().nnodes(), 4);
...
TEST(DeepseekV32CPUtilsTest, AsyncGatherHelpersMatchBlockingGather) {
  ScopedFlagValue nnodes(DistributedConfig::get_instance().nnodes(), 4);
