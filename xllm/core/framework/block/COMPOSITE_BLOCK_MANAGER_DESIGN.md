...
- **三种 sub-manager 类型**：
  ...
  3. **FixedBlockManager**：每 Sequence 仅分配 1 个 block；已分配则直接返回该 block，直到 Sequence 释放才归还。
...
classDiagram
  ...
  BlockManager <|-- FixedBlockManager
...
- **SlidingWindowBlockManager** / **FixedBlockManager** 实现 **BlockManager**，作为 Composite 的 sub-manager；具体分配逻辑封装在各自类的 `allocate_for_sequence` 中。
...
- **职责**：每个 Sequence 至多 1 个 block；已分配则返回同一 block（幂等）；Sequence 释放时归还该 block。逻辑全部封装在本类内，不在 BlockManager 基类增加 Fixed 专用接口。
- **实现**：实现 **BlockManager** 并重写 **allocate_for_sequence(Sequence* seq, size_t num_tokens)**。内部维护 `Sequence* -> Block`（或 sequence_id -> Block）映射；`allocate_for_sequence` 若该 seq 已有块则返回 `{ cached_block }`，否则 `allocate(1)` 得到一块、插入 map 后返回；**deallocate(blocks)** 被调用时（由 Composite 在释放 sequence 时传入该 slot 的 blocks）从 map 中反查并移除对应 Sequence，并调用底层 free。Composite 对所有 sub-manager 统一调用 `allocate_for_sequence(seq, num_tokens)`，不区分类型。

**文件**：新建 `fixed_block_manager.h` / `fixed_block_manager.cpp`。

---

## 六、CompositeBlockManager（实现 BlockManager，由 Pool 管理）

- **职责**：实现 **BlockManager**，由 **BlockManagerPool** 在 `block_managers_` 中持有；内部持有多个 **BlockManager**（BlockManagerImpl、SlidingWindowBlockManager、FixedBlockManager）。Pool 通过 `is_composite()` 分支调用本类的 **allocate_for_sequence** / **deallocate_sequence**，本类在多个 sub-manager 上统一按「按 Sequence 分配」语义协同分配/释放，结果写入 **Sequence 的 kv_state** 中的 `composite_blocks_`。
...
- **allocate_for_sequence(seq, num_tokens)**（重写 BlockManager 的同一接口）：
  - 对每个 slot i 统一调用 `blocks = sub_managers_[i]->allocate_for_sequence(seq, num_tokens)`（各 sub-manager 内部封装自己的逻辑：Normal 按 token 数、SlidingWindow 固定 window_size、Fixed 每 Sequence 一块幂等），将结果写入 `seq->kv_state().mutable_composite_blocks()`（需保证 size 至少为 sub_managers_.size()，再写第 i 项）。若某 slot 已为该 sequence 分配过且不需补足（如 SlidingWindow/Fixed），由各 sub-manager 的 `allocate_for_sequence` 实现决定返回已有 blocks 或新分配。
...
## 七、BlockManager 接口扩展（统一「按 Sequence 分配」）

- **不为 FixedBlockManager 单独增接口**：所有 BlockManager 的语义都是「为某个 Sequence 分配 blocks」，具体逻辑（按 token 数、固定 window_size、每 Sequence 固定 1 块等）封装在各自类内部。
- 在 `block_manager.h` 中只增加统一的「按 Sequence 分配」入口及 Composite 与 Pool 的协作接口：
  ...
  - **`virtual std::vector<Block> allocate_for_sequence(Sequence* seq, size_t num_tokens)`**  
    ...
    - **FixedBlockManager**：重写为内部维护 Sequence→Block 映射，已分配则返回该 block，否则 allocate(1) 并缓存（逻辑封装在类内）。  
...
- **BlockManagerImpl**、**ConcurrentBlockManagerImpl** 不重写；**SlidingWindowBlockManager** / **FixedBlockManager** 只重写 `allocate_for_sequence`，各自封装策略。
...
- **依赖**：CompositeBlockManager 依赖 KVCacheState 的 `composite_blocks_`，故需先在 sequence_kv_state 中扩展并已在 KVCacheState::reset() 中清空；SlidingWindowBlockManager / FixedBlockManager 仅依赖 block 与 BlockManager，可与 Composite 同批或先行。
- **测试**：为 CompositeBlockManager、SlidingWindowBlockManager、FixedBlockManager 各写单元测试；并测试 Pool 在某一 rank 使用 CompositeBlockManager 时的 allocate(sequence, num_tokens) / deallocate(sequence) 路径。
...
2. **BlockManager 接口扩展**：增加 `is_composite()`、`allocate_for_sequence(seq, num_tokens)`、`deallocate_sequence(seq)` 默认实现；各子类只重写 `allocate_for_sequence` 封装自身逻辑，不新增 Fixed 专用接口。
3. **SlidingWindowBlockManager**：实现 BlockManager，重写 `allocate_for_sequence` 为 `return allocate(window_size);`，接入 CMakeLists。
4. **FixedBlockManager**：实现 BlockManager，重写 `allocate_for_sequence` 封装「每 Sequence 一 block、幂等」逻辑，接入 CMakeLists。
5. **CompositeBlockManager**：实现 BlockManager（含 is_composite/allocate_for_sequence/deallocate_sequence），对每个 sub_manager 统一调用 `allocate_for_sequence(seq, num_tokens)`，写入 `seq->kv_state().mutable_composite_blocks()` 并可选同步主 slot 到 kv_state_ 的 blocks_。
6. **BlockManagerPool 改动**：在 allocate(sequence, num_tokens) 与 deallocate(sequence) 中根据 `is_composite()` 分支调用 `allocate_for_sequence` / `deallocate_sequence`。
7. **集成与测试**：在创建 block_managers_ 的路径中支持配置 CompositeBlockManager，并补充单测。
...
- 所有 BlockManager 统一通过 **allocate_for_sequence(seq, num_tokens)** 表达「为 Sequence 分配 blocks」，具体逻辑封装在各自类内：BlockManagerImpl 默认按 token 数；SlidingWindowBlockManager 固定 window_size；FixedBlockManager 每 Sequence 一 block、幂等。不在基类为 Fixed 单独增接口。
