...
class KVCacheStore {
 public:
  ...
  uint64_t batch_put(Slice<BlockTransferInfo>& block_transfer_info);

  uint64_t batch_get(Slice<BlockTransferInfo>& block_transfer_info);

  uint64_t batch_remove(Slice<BlockTransferInfo>& block_transfer_info);

  uint64_t batch_exist(std::vector<std::string>&& keys);
