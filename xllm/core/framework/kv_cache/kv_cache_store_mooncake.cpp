...
MooncakeStore::MooncakeStore(StoreConfig& config) : KVCacheStore(config) {
  ...
  if (config_.protocol == "rdma") {
    for (int layer = 0; layer < config_.host_kv_caches->size(); layer++) {
      void* key_cache = static_cast<char*>(
          config_.host_kv_caches->at(layer).get_k_cache().data_ptr());
      ...
      void* value_cache = static_cast<char*>(
          config_.host_kv_caches->at(layer).get_v_cache().data_ptr());
...
uint64_t MooncakeStore::batch_put(
  ...
  for (auto block_info : cache_block_info) {
    ...
    slice.reserve(config_.host_kv_caches->size() * 2);
    for (int layer = 0; layer < config_.host_kv_caches->size(); layer++) {
      void* key_cache =
          static_cast<char*>(
              config_.host_kv_caches->at(layer).get_k_cache().data_ptr()) +
...
uint64_t MooncakeStore::batch_get(
    const std::vector<CacheBlockInfo>& cache_block_info) {
  ...
  for (auto block_info : cache_block_info) {
    ...
    slices[str_key].reserve(config_.host_kv_caches->size() * 2);
    for (int layer = 0; layer < config_.host_kv_caches->size(); layer++) {
      void* key_cache =
          static_cast<char*>(
              config_.host_kv_caches->at(layer).get_k_cache().data_ptr()) +
...
uint64_t MooncakeStore::batch_remove(
    const std::vector<CacheBlockInfo>& cache_block_info) {
