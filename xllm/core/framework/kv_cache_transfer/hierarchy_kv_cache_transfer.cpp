...
bool HierarchyKVCacheTransfer::finalize_registration() {
  ...
  if (options_.host_blocks_factor() > 1.0) {
    ...
    config.record_completion_event = cache_domains_.size() > 1;
...
HostKVRequest HierarchyKVCacheTransfer::make_request(
  ...
  request.completion_mappings.reserve(block_transfer_info.size());
  for (const BlockTransferInfo& info : block_transfer_info) {
    ...
    for (const CacheDomain& domain : cache_domains_) {
      ...
      std::vector<HostKVMapping>& mappings =
          is_load && domain.role == CacheRole::DRAFT
              ? request.completion_mappings
              : request.mappings;
...
void HierarchyKVCacheTransfer::set_layer_synchronizer(
  ...
  params.parallel.kv_cache_load_wait_policy = KVCacheLoadWaitPolicy::LAYER_WISE;
  params.parallel.completion_event_index = handle->completion_event_index;
}
