...
class BasicHostKVTransfer::Impl final {
 public:
  ...
  HostKVLoadHandle prepare_load() const {
    const uint32_t primary_event_count = static_cast<uint32_t>(ranges_.size());
    const uint32_t completion_event_index = record_completion_event_
                                                ? primary_event_count
  ...
  bool load(const HostKVRequest& request, const HostKVLoadHandle& handle) {
    ...
    for (size_t index = 0; index < ranges_.size(); ++index) {
      CopyPlan plan = build_plan(
          layout_, request.mappings, ranges_[index], /*is_load=*/true);
    ...
    for (const LayerRange& range : ranges_) {
      CopyPlan plan = build_plan(
          layout_, request.completion_mappings, range, /*is_load=*/true);
      if (!plan.src_tensors.empty() &&
          !batch_memcpy_->submit_h2d(
              plan.src_tensors, plan.dst_tensors, stream.get())) {
        if (stream_has_work) {
          stream.drain_or_die("completion copy submission failed");
        }
        return false;
      }
    ...
    if (record_completion_event_ &&
      ...
      if (stream_has_work) {
        stream.drain_or_die("completion event recording failed");
      }
      return false;
    }
    return true;
  ...
  bool offload(const HostKVRequest& request) {
    ...
    for (const LayerRange& range : ranges_) {
      CopyPlan plan =
          build_plan(layout_, request.mappings, range, /*is_load=*/false);
      if (!plan.src_tensors.empty() &&
