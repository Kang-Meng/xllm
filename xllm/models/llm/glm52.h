...
class Glm52ModelImpl : public Glm5ModelImpl {
 ...
 protected:
  // Both eager execution and graph capture derive DCP metadata in forward.
  // Captured operations then refresh it from persistent graph inputs on replay.
  void prepare_attention_metadata(
    ...
    attn_metadata.slot_mapping = mark_graph_padding_slots_invalid(
        input_params, attn_metadata.slot_mapping);
    attn_metadata.kv_shard_batch_metadata =
        build_mlu_shard_metadata(attn_metadata, kv_shard_layout_.value());
  }
 ...
 private:
  // Graph buckets pad decode rows with slot 0, which stays a physically valid
  // cache slot for the generic safe-block-row conventions. DCP must instead
  // treat padded rows as no-write rows, so remap them to the invalid-slot
  // sentinel on device. Metadata preparation refreshes this remap from the
  // persistent valid-row count before every graph capture or replay. Undefined
  // outside graph mode, where no bucket padding exists.
  static torch::Tensor mark_graph_padding_slots_invalid(
      const ModelInputParams& input_params,
      const torch::Tensor& slot_mapping) {
    const torch::Tensor& num_valid_token_rows =
        input_params.graph.num_valid_token_rows;
    if (!num_valid_token_rows.defined()) {
      return slot_mapping;
    }
    CHECK(slot_mapping.defined())
        << "GLM-5.2 DCP graph slot masking requires slot mapping";
    CHECK_EQ(num_valid_token_rows.numel(), 1)
        << "GLM-5.2 DCP graph valid-row count must be a scalar";
    const torch::Tensor row_ids =
        torch::arange(slot_mapping.numel(), slot_mapping.options());
    const torch::Tensor padding_rows = row_ids >= num_valid_token_rows;
    return torch::where(
        padding_rows,
        torch::full_like(slot_mapping, KVShardLayout::kInvalidSlot),
        slot_mapping);
  }
