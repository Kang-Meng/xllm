...
def pool_key_indexer(
    ...
    *,
    ...
    if layout_k == "BSND":
        ...
        if pool_key.dim() != 3 or pool_key.size(1) != 1 or pool_key.size(-1) != 128 or pool_key.numel() == 0:
        ...
        if pool_key.dim() != 4 or pool_key.size(2) != 1 or pool_key.size(-1) != 128 or pool_key.numel() == 0:
    ...
    if query.device.type != "npu" or any(tensor.device != query.device for tensor in inputs):
    ...
    if (
        actual_seq_k is not None
        and layout_k == "TND"
        and actual_seq_k.numel()
        not in (
            batch_size,
            batch_size + 1,
        )
    ...
    try:
        ...
        raise NotImplementedError("CANN pool_key_indexer is unavailable in this runtime") from exc
    return op(
        query,
        pool_key,
        weights,
